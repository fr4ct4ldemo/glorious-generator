...
from sqlalchemy import (
    Column, String, Integer, Boolean, Text, func, JSON, delete, text
)
...
class Account(Base):
    ...
    guild_id = Column(String, nullable=False, default="0", index=True)
...
class Review(Base):
    ...
    liked_by = Column(MutableDict.as_mutable(JSON), default={})
...
async def init_db():
    async with engine.begin() as conn:
        ...
        # Ensure guild_id column exists for backward compatibility
        try:
            await conn.execute(text("ALTER TABLE accounts ADD COLUMN guild_id VARCHAR DEFAULT '0'"))
        except Exception:
            pass  # Column already exists, ignore
        # Update any NULL guild_id values to '0'
        try:
            await conn.execute(text("UPDATE accounts SET guild_id = '0' WHERE guild_id IS NULL"))
        except Exception:
            pass

async def getServices(guild_id: str):
    async with Session() as session:
        services = (await session.execute(select(Account.service_name).distinct().filter_by(guild_id=guild_id)))
        return services.scalars().all()

async def deleteService(service_name, guild_id):
    async with Session() as session:
        stmt = delete(Account).where((Account.service_name == service_name.lower()) & (Account.guild_id == guild_id))
        await session.execute(stmt)
        await session.commit()
        return True

async def getAccount(service_name, guild_id):
    combo = ""
    async with Session() as session:
        account = (await session.execute(select(Account).filter_by(service_name=service_name.lower(), guild_id=guild_id).order_by(func.random()))).scalars().first()
        if account:
            combo = account.combo
...
async def getMultipleAccounts(service_name: str, amount: int, guild_id: str):
    combos = []
    async with Session() as session:
        query = (
            select(Account)
            .filter_by(service_name=service_name.lower(), guild_id=guild_id)
...
async def addStock(service_name, stock, remove_capture, guild_id):
    async with Session() as session:
        existing_combos = (
            await session.execute(
                select(Account.combo).filter_by(service_name=service_name.lower(), guild_id=guild_id)
            )
        ).scalars().all()
        ...
        for account in stock:
            ...
            if combo in existing_combos_set:
                ...
                added_accounts.append(Account(service_name=service_name.lower(), combo=combo, guild_id=guild_id))
...
async def getStock(service_list, guild_id):
    async with Session() as session:
        stock = []
        for service in service_list:
            count = (await session.execute(select(func.count(Account.id)).filter_by(service_name=service.lower(), guild_id=guild_id))).scalar()
...
async def get_role_user_cooldown(
    ...
    async with Session() as session:
        ...
        user = (
            await session.execute(
                select(User).filter_by(user_id=str(interaction.user.id))
            )
        ).scalars().first()
        if not user:
            return None
        ...
        for role in guild_roles:
            if is_premium:
                if (
                    int(role["id"]) in userRoles
                    and float(role.get("premium-cooldown", default_cooldown)) < minCooldown
                ):
                    minCooldown = float(role.get("premium-cooldown", default_cooldown))
            else:
                if (
                    int(role["id"]) in userRoles
                    and float(role.get("free-cooldown", default_cooldown)) < minCooldown
                ):
                    minCooldown = float(role.get("free-cooldown", default_cooldown))

        if user and user.custom_cooldown:
            custom = user.custom_cooldown.get(stage_key)
            if custom is not None:
                minCooldown = float(custom)

        current_time = datetime.now().timestamp()
        cooldown_end = current_time + int(
            minCooldown if minCooldown != float("inf") else default_cooldown
        )
...
# Review functions
async def create_review(guild_id: str, user_id: str, username: str, avatar_url: str, review_text: str, rating: int):
    async with Session() as session:
        review = Review(
            guild_id=guild_id,
            user_id=user_id,
...
async def get_review(review_id: int):
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(id=review_id))).scalars().first()
        return review

async def update_review_message_id(review_id: int, message_id: str):
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(id=review_id))).scalars().first()
        if review:
...
async def add_like_to_review(review_id: int, user_id: str):
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(id=review_id))).scalars().first()
        if review:
            if review.liked_by is None:
                review.liked_by = {}
            if str(user_id) not in review.liked_by:
                review.liked_by[str(user_id)] = True
                review.likes += 1
                await session.commit()
                return True
        return False

async def remove_like_from_review(review_id: int, user_id: str):
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(id=review_id))).scalars().first()
        if review:
            if review.liked_by and str(user_id) in review.liked_by:
                del review.liked_by[str(user_id)]
                review.likes = max(0, review.likes - 1)
                await session.commit()
                return True
        return False

async def user_already_liked(review_id: int, user_id: str) -> bool:
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(id=review_id))).scalars().first()
        if review and review.liked_by:
            return str(user_id) in review.liked_by
        return False

async def get_user_today_review(guild_id: str, user_id: str):
    async with Session() as session:
        today = datetime.utcnow().date().isoformat()
        reviews = (await session.execute(
            select(Review).filter_by(guild_id=guild_id, user_id=user_id)
        )).scalars().all()
        
        for review in reviews:
            review_date = datetime.fromisoformat(review.timestamp).date().isoformat()
            if review_date == today:
                return review
        return None

async def get_review_by_message_id(message_id: str):
    async with Session() as session:
        review = (await session.execute(select(Review).filter_by(message_id=message_id))).scalars().first()
        return review
