...
async def gen(
    ...
    if not success:
        ...
        try:
            ...
            embd = discord.Embed(
                title=f"{EMOJIS['gen']} Account Generated",
                description=f"{EMOJIS['key']} Check your DMs for your account!",
                color=NEON_GREEN,
            )
            embd.set_footer(
    ...
    try:
        ...
        review_embed = discord.Embed(
            ...
            description="Enjoyed the service? Click the button below to leave a review!",
            color=NEON_GREEN
        )
        review_embed.set_footer(text=config["messages"]["footer-msg"])
        
        view = ReviewPromptView(interaction.guild_id)
        return await channel.send(embed=review_embed, view=view)
        ...
        return await interaction.followup.send(
            content=f"{interaction.user.mention}, couldn't send you a DM, open your DMs!",
            ephemeral=True,
        )
